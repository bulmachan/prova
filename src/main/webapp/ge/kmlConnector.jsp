...
class kmlConnector{
	...
	public String ipSvil = "10.10.80.35";
	
	public String oggi = String.format("%1$td%1$tm%1$tY", new java.util.Date());	
	...
	public void cNetworkLink(double xMin, double xMax, double yMin, double yMax, int level, StringBuffer sb, String number, int nPoints, String servletURL, int i, String minLod, String maxLod, int liv){
		...
		sb.append("\t\t<name>Cartografia " + number+String.valueOf(i)+"." + "</name>\r\n");
	...
	public void toPlacemark(StringBuffer sb, String sdeUser, String layer, String[] colonne, String connUser, String connPw, String nomeServer, int portaSDE, SeConnection connSDE, int scalaGen, String stylesPath,StringBuffer textLog, String filtro){
		micSpatialEngine mSE = new micSpatialEngine(connUser,connPw,nomeServer,portaSDE);
		try{
			ArrayList risultato=new ArrayList();
			
			//mSE.trovaElementoPerParametri(sdeUser+"."+layer, "OBJECTID = 1296", connSDE);
			
			
			// SENTIERI:
			if(layer.equals("ESC_VF_SENPERCORSI_LIN"))
				risultato=mSE.getElementi(sdeUser+"."+layer, colonne, connSDE, filtro);
			

			// PUNTI
			if(layer.equals("ESC_VF_POI_GE_PUN"))
				risultato=mSE.getElementi(sdeUser+"."+layer, colonne, connSDE, filtro);


			//sb.append("HHHH"+mSE.getReport());

			/*String[] colonneLin={"SHAPE"};
			risultato=mSE.getElementi(sdeUser+"."+"ESC_VF_SENPERCORSI_LIN", colonneLin, connSDE, "LOCALITA='ALTA VIA DEI PARCHI - 1^ TRATTA - BERCETO-LAGO SANTO PARMENSE'");
			sb.append("HHHH"+mSE.getReport());
			//if(mSE.
			risultato=mSE.getElementi(sdeUser+"."+layer, colonne, connSDE);
			sb.append("BBBBB"+mSE.getReport());*/
			...
			try{
				...
				for(int j=0; j<risultato.size(); j++){
					...
					sb.append("\t\t\t<Placemark>"+"\r\n");
					if(ris[3].equals("")){
						sb.append("\t\t\t\t<name><![CDATA["+ris[2]+"]]></name>"+"\r\n");
					} else {
							sb.append("\t\t\t\t<name><![CDATA["+ris[3]+"]]></name>"+"\r\n");
					}
					...
					if(ris.length>5)
						if(ris[5].equals(""))
							...
							if(ris[5].substring(0,7).equals("http://"))
								sb.append("\t\t\t\t<description><![CDATA["+ris[4]+"<br /><a href='"+ris[5]+"'>Apri pagina WEB</a>]]></description>"+"\r\n");
							else
								sb.append("\t\t\t\t<description><![CDATA["+ris[4]+"<br />"+ris[5]+"]]></description>"+"\r\n");
					else
						sb.append("\t\t\t\t<description><![CDATA["+ris[4]+"]]></description>"+"\r\n");
					...
					if (ris[0].substring(0,ris[0].indexOf(" (")).equals("MULTILINESTRING")){
						...
						strCoords=ris[0].replace("MULTILINESTRING (( ","");
					...
					}else if (ris[0].substring(0,ris[0].indexOf(" (")).equals("LINESTRING")){
						
						strCoords=ris[0].replace("LINESTRING ( ","");
						strCoords=strCoords.replace(")","");
							
						String[] coordsList=strCoords.split(", ");
	
						sb.append("\t\t\t\t<MultiGeometry id=\"MM"+ris[1]+"\">"+"\r\n");
						sb.append("\t\t\t\t<LineString id=\""+ris[1]+"\">"+"\r\n");
						sb.append("\t\t\t\t\t<coordinates>");

						//if (scalaGen>0)
						//	coordsList=generalize(coordsList,scalaGen);
						String[] puntiLinea=new String[3];
						int jjjj=0;
						
						int primoPuntoIdx=coordsList.length/4;
						int secondoPuntoIdx=coordsList.length/2;
						int terzoPuntoIdx=(coordsList.length/2)+(coordsList.length/4);

						for(int jjj=0; jjj<coordsList.length; jjj++){
							String[] coord=coordsList[jjj].split(" ");
							...
							if(jjj==primoPuntoIdx || jjj==secondoPuntoIdx || jjj==terzoPuntoIdx){
								puntiLinea[jjjj]=String.valueOf(llWgs84[1])+","+String.valueOf(llWgs84[0])+",0"+" ";
								jjjj++;
							}
						...
						sb.append("\t\t\t\t<Point>"+"\r\n");
						sb.append("\t\t\t\t<coordinates>"+puntiLinea[0]+"</coordinates>"+"\r\n");
						sb.append("\t\t\t\t</Point>"+"\r\n");
						sb.append("\t\t\t\t<Point>"+"\r\n");
						sb.append("\t\t\t\t<coordinates>"+puntiLinea[1]+"</coordinates>"+"\r\n");
						sb.append("\t\t\t\t</Point>"+"\r\n");
						sb.append("\t\t\t\t<Point>"+"\r\n");
						sb.append("\t\t\t\t<coordinates>"+puntiLinea[2]+"</coordinates>"+"\r\n");
						sb.append("\t\t\t\t</Point>"+"\r\n");
						
						sb.append("\t\t\t\t</MultiGeometry>"+"\r\n");

					}else if (ris[0].substring(0,ris[0].indexOf(" (")).equals("MULTIPOLYGON")){
					...
					}else if (ris[0].substring(0,ris[0].indexOf(" (")).equals("POLYGON")){
					...
					}else if (ris[0].substring(0,ris[0].indexOf(" (")).equals("POINT")){

						strCoords=ris[0].replace("POINT ( ","");
						strCoords=strCoords.replace(")","");
							
						String[] coordsList=strCoords.split(", ");
						
						sb.append("\t\t\t\t<Point id=\""+ris[1]+"\">"+"\r\n");
	...
	public void showRegion(double xMin, double xMax, double yMin, double yMax, StringBuffer sb, String appPath, String sdeUser, String layer, String layerlin, String layerlin2, int scalaGen, String number, int i, int level, String minLod, String maxLod, SeConnection connSDE, int nPoints, String[] colonne, String[] colonnelin, String connUser, String connPw, String nomeServer, int portaSDE, StringBuffer textLog, String descEtichetta, String balloonWidth, String balloonHeight, String stylesPath, String ip){
		...
		try{
			...
			if(mSE.getReport().indexOf("Error")>-1){
				...
				erroreServer=" [Poligoni al momento non disponibili, riprovare più tardi]";
				log.error(textLog);
			}

			if(!layerlin2.equals("") && level>2)
				layerlin=layerlin2;
			
			ArrayList risultatolin=new ArrayList();
			int scalaGenLin=scalaGen/5;
			if(!layerlin.equals("")){
				if(i>0){
					risultatolin=mSE.tagliaElementiPerBox(sdeUser+"."+layerlin, colonnelin, xMin, yMin, xMax, yMax, connSDE);
				} else {
					risultatolin=mSE.getElementi(sdeUser+"."+layerlin, colonnelin, connSDE);
				}
			}
			...
			if(mSE.getReport().indexOf("Error")>-1){
				textLog.append(mSE.getReport()+" - ");
				erroreServer+=" [Linee al momento non disponibili, riprovare più tardi]";
				log.error(textLog);
			}
			...
			String strCoords="";
			sb.append("<Folder>"+"\r\n");
			//sb.append("<name>Geometrie "+number+String.valueOf(i)+"."+" &#40;"+String.valueOf(nPoints)+"&#41;"+"</name>"+"\r\n");
			if (i>0)
				sb.append("<name>Geometrie "+number+String.valueOf(i)+"."+erroreServer+"</name>"+"\r\n");
			else
				sb.append("<name>Geometrie"+erroreServer+"</name>"+"\r\n");
			...
			if(!layerlin.equals("")){
				sb.append("<Folder>"+"\r\n");
				sb.append("<name>Limiti</name>"+"\r\n");
				sb.append("<visibility>1</visibility>"+"\r\n");
				sb.append("<open>0</open>"+"\r\n");
				try{
					for(int j=0; j<risultatolin.size(); j++){
						
						String[] ris = (String[]) risultatolin.get(j);
						
						// LINEA
						sb.append("\t<Placemark>"+"\r\n");
						sb.append("\t\t<name>Limite di Unità cartografica</name>"+"\r\n");
						sb.append("\t\t<visibility>1</visibility>"+"\r\n");
						...
						if (ris[0].indexOf("MULTILINESTRING")>-1){
							...
							strCoords=strCoords.replace(xMin+" "+yMin+", "+xMax+" "+yMin+", ","");
							strCoords=strCoords.replace(xMax+" "+yMin+", "+xMin+" "+yMin+", ","");

							strCoords=strCoords.replace(xMin+" "+yMax+", "+xMax+" "+yMax+", ","");
							strCoords=strCoords.replace(xMax+" "+yMax+", "+xMin+" "+yMax+", ","");

							strCoords=strCoords.replace(xMax+" "+yMax+", "+xMax+" "+yMin+", ","");
							strCoords=strCoords.replace(xMax+" "+yMin+", "+xMax+" "+yMax+", ","");
							...
							for(int jj=0; jj<partsList.length; jj++){
								...
								if (partOutInList.length>0){
									...
									if (scalaGenLin>5000)
										coordsPartOutList=generalize(coordsPartOutList,scalaGenLin);
								...
								for(int jjjj=1; jjjj<partOutInList.length; jjjj++){
									...
									if (scalaGenLin>5000)
										coordsPartInnList=generalize(coordsPartInnList,scalaGenLin);
			...
			try{
				...
				for(int j=0; j<risultato.size(); j++){
					...
					if (ip.equals(ipSvil))
						sb.append("\t\t<description><![CDATA[<iframe src=\"" + ris[3].replace("http://geotest.ente.regione.emr.it","http://"+ipSvil+"/geologico") + "\" width=\""+balloonWidth+"\" height=\""+balloonHeight+"\" frameborder=\"0\" marginwidth=\"0\" marginheight=\"0\" name=\"balloon\"></iframe>" + "]]></description>" + "\r\n");
					else
	...
	public void appendLicenza(StringBuffer sb){
		sb.append("\t<Folder>"+"\r\n");
		sb.append("\t\t<name>Licenza</name>"+"\r\n");
		sb.append("\t\t<visibility>1</visibility>"+"\r\n");
		sb.append("\t\t<open>0</open>"+"\r\n");
		...
		sb.append("\t\t\t<screenXY x=\"45\" y=\"25\" xunits=\"pixels\" yunits=\"pixels\"/>"+"\r\n");
